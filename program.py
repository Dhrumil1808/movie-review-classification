...
training_labels = [int(l[:2]) for l in lines]
...
similarity_matrix = cosine_similarity(mat_normalize,dense_output=False)
...
for test_review_index in range(training_data, training_data +testing_data):
    similarity = similarity_matrix[test_review_index, :training_data].toarray().tolist()[0]
    similar_labels = zip(similarity, training_labels, range(len(training_labels)))
 	sort_similar_lables = sorted(similar_labels, key=lambda (val, k, l): val, reverse=True)
           
    count=0;
	for j in range(k):
        if sort_similar_labels[j][0] != 0:
            count += int(sort_similar_labels[j][1])
        if count == 0:
            while count == 0:
